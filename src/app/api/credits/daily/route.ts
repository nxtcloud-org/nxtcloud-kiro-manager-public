...
import { getDailyCredits } from "@/services/credit.service";
...
export async function GET(request: NextRequest) {
  ...
  const data = await getDailyCredits({ start, end, groupCodes });
  return NextResponse.json(
    data.map((d) => ({ date: d.date, credits: d._sum.creditsUsed ?? 0 })),
  );
}
